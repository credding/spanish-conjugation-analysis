...
def main() -> None:
    ...
    dle_lemmas = load_dle_lemmas()
    dle_verb_forms = load_dle_verb_forms()

    ctx = Context(corpes, dle_lemmas, dle_verb_forms)
...
class Context:
    def __init__(
        self,
        corpes: CORPES,
        dle_lemmas: dict[Lemma, DLELemma],
        dle_verb_forms: dict[Verb, list[DLEVerbForm]],
    ) -> None:
        self._corpes = corpes
        self._dle_lemmas = dle_lemmas
        self._dle_verb_forms = dle_verb_forms
    ...
    def _index_freq_lemma_verb(self, freq_lemma: FreqLemma) -> TaggedVerb:
        dle_verb = cast("DLEVerb", self._dle_lemmas[Verb(freq_lemma.base_form)])
    ...
    def index_dle_verb_forms(self) -> None:
        ...
        dle_forms = [
            fx
            for vx in self._grammar_index.lookup_verbs()
            for fx in self._dle_verb_forms[vx.key]
        ]
    ...
    def lookup_dle_verb_form_homonym_lemmas(self) -> None:
        ...
        for lemma_tag in homonym_lemma_tags:
            dle_lemma = self._dle_lemmas.get(lemma_tag)
